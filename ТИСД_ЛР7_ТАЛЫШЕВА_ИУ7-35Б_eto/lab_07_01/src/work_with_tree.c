...
void tree_print(struct Node *Head_tree, char *name_file)
{
    ...
    // printf("tyt\n");
    btree_export_to_dot(f, "visual_graph", Head_tree);
    fclose(f);
}
...
struct Node *tree_find_elem(struct Node *Head_tree, int num, int *count)
{
    ...
    {
        *count += 1;
        return tree_find_elem(Head_tree->left, num, count);
    }
    else
    {
        *count += 1;
        return tree_find_elem(Head_tree->right, num, count);
    }
}
...
double tree_count_average_find(struct Node *Head_tree, FILE *file)
{
    ...
    {
        count = 0;
        if (tree_find_elem(Head_tree, x, &count) != NULL)
        {
            //printf("num = %d count = %d\n", x, count);
            // Увеличиваем res_count только если элемент найден
            res_count += count;
            count_num++;
            //printf("    res = %d k_num = %d\n", res_count, count_num);
        }
    }
    fseek(file, 0, SEEK_SET);
    double res = 0.0;
    if (count_num != 0)
        res = (double) res_count / (double) count_num;
    return res;
}